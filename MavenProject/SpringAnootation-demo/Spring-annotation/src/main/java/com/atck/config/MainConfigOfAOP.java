...
import com.atck.aop.LogAspects;
...
/**
 * AOP:【动态代理】
 *      指在程序运行期间动态的将某段代码切入到入道指定的方法的指定位置运行的编程方式
 * 1.导入AOP模块：Spring aop
 * 2.定义一个业务逻辑类：（MathCalculator），在业务逻辑运行的时候将日志进行打印（方法之前、方法运行结束、方法出现异常、。。。。）
 * 3.定义一个日志切面类（LogAspects），切面里面的方法需要动态感知MathCalculator.div运行到哪里了然后执行
 *      通知方法：
 *              前置通知(@Before)：logStart，在目标方法（div）运行之前运行
 *              后置通知(@After)：logEnd，在目标方法（div）结束之后运行(无论方法是正常结束还是异常结束)
 *              返回通知(@AfterReturning)：logReturn，在目标方法（div）正常返回之后运行
 *              异常通知(@AfterThrowing)：logException，在目标方法（div）出现异常之后运行
 *              环绕通知(@Around)：动态代理，手动推进目标方法运行（joinpoint.procceed()）
 * 4.给切面类的目标方法标注何时何地运行（通知注解）
 * 5.将切面类和业务逻辑类（目标方法所在类）都加入到容器中
 * 6.必须告诉spring哪个类是切面类（给切面类加上注解：@Aspect）
 * [7].给配置类中加@EnableAspectJAutoProxy【开启基于注解的aop模式】
 *      在spring中有很多@Enablexxxx，开启某一个功能
 *
 * 三步
 *      1.将业务逻辑组件和切面类都加入到容器中，告诉Spring那个是切面类
 *      2.在切面类的每一个通知方法上加上通知注解，告诉Spring何时何地运行（切入点表达式）
 *      3.开启基于注解的aop模式
 * AOP原理【看给容器中注入了什么组件，什么时候工作，这个组件的功能是什么】
 ...
 * AbstractAutoProxyCreator.postProcessBeforeInstantiation有后置处理器的逻辑
 *
 *
 ...
 *              getBean(beanName); ==> doGetBean(name, null, null, false); ==> getSingleton(beanName, new ObjectFactory<Object>()
 *      2.创建bean
 *          【AnnotationAwareAspectJAutoProxyCreator在所有bean创建之前会有一个拦截，InstantiationAwareBeanPostProcessor，会调用postProcessBeforeInstantiation】
 *              1.先从缓存中获取当前bean，如果能获取到，说明bean是之前创建过的，直接使用，否则再创建
 *                      创建好的bean都会被缓存起来
 *              2.createBean(beanName, mbd, args);创建bean,AnnotationAwareAspectJAutoProxyCreator会在任何bean创建之前尝试返回bean
 *                  【BeanPostProcessor是在bean对象创建完成初始化前后调用的】
 *                  【InstantiationAwareBeanPostProcessor是在创建bean实例之前先尝试用后置处理器返回对象】
 *                      1.resolveBeforeInstantiation(beanName, mbdToUse);解析BeforeInstantiation
 *                              希望后置处理器在此返回一个代理对象，如果能返回代理对象就是用，如果不能就继续
 *                              1.后置处理器先尝试返回对象
 ...
 *
 * AnnotationAwareAspectJAutoProxyCreator是 InstantiationAwareBeanPostProcessor的实现类
