 ...
 *      postProcessBeanDefinitionRegistry().在所有bean定义信息将要被加载，备案实例还未被创建的时候执行
 *
 ...
 *      4.再来从容器中找到方法BeanFactoryPostProcessor组件，然后依次触发postProcessorBeanFactory()方法
 ...
 *      com.atck.test.IOCTest_ext$1[source=我发布的事件]
 *      ContextClosedEvent
 *  1.ContextRefreshedEvent
 *      1.容器创建对象：refresh()
 *      2.finishRefresh();容器刷新完成会发布ContextRefreshedEvent事件
 *  2.自己发布的事件
 *  3.容器关闭会发布publishEvent(new ContextClosedEvent(this));
 *              事件发布流程
 *       3.publishEvent(new ContextRefreshedEvent(this));
 *                  1.获取事件派发器：getApplicationEventMulticaster()
 *                  2.multicastEvent(applicationEvent, eventType);派发事件
 *                  3.获取到所有的AoolicationListener
 *                          for (final ApplicationListener<?> listener : getApplicationListeners(event, type))
 *                              1.如果有Executor，可以支持使用异步派发
 ...
 *              1.先创建所有的单实例备案，getBean()
 *              2.获取所有创建好的单实例bean，判断是否是SmartInitializingSingleton类型的
 *                  如果是就调用afterSingletonsInstantiated();
 *
 */
@Configuration
@ComponentScan("com.atck.ext")
